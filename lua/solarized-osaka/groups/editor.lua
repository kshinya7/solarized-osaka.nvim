...
function M.get(c, options)
  ...
  return {
    ...
    DiffAdd                     = { bg = c.green900 }, -- diff mode: Added line |diff.txt|
    DiffChange                  = { bg = c.yellow900 }, -- diff mode: Changed line |diff.txt|
    DiffDelete                  = { bg = c.red900 }, -- diff mode: Deleted line |diff.txt|
    DiffText                    = { bg = c.blue900 }, -- diff mode: Changed text within a changed line |diff.txt|
    DiffAddText                 = { bg = c.green800 }, -- diff mode: Added text within a changed line
    DiffDeleteText              = { bg = c.red700 }, -- diff mode: Deleted text within a changed line
    ...
    LineNr                      = { fg = c.yellow800, bg = options.transparent and c.none or c.bg }, -- Line number for ":number" and ":#" commands, and when 'number' or 'relativenumber' option is set.
    ...
    NormalNC                    = { fg = c.fg, bg = options.transparent and c.none or options.dim_inactive and c.base04 or c.bg }, -- normal text in non-current windows
